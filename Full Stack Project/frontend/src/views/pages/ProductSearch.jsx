...
function ProductSearch() {
  ...
  useEffect(
    ...
    [dispatch, categoryParams, productName]
  );
  ...
  useEffect(
    ...
    [dispatch, categoryParams, productName, lowestPrice, HighestPrice]
  );
  ...
  if (categoryParams)
    productsAll = productsAll.filter(
      (each) => each.category === categoryParams
    );

  if (productName)
    productsAll = productsAll.filter(
      (each) => each.slug.toLowerCase().indexOf(productName.toLowerCase()) > -1
    );
  ...
  return (
    <div>
      <PageTopBackImg>Search Product </PageTopBackImg>
